    ...
    if list_notes.selectedItems():
        ...
        for note in notes:
            if note[0] == key:
                ...
                with open("notes4/"+str(index)+".txt", "w") as file:
                    file.write(note[0]+'\n')
                    file.write(note[1]+'\n')
                    for tag in note[2]:
                        file.write(tag+' ')
...
note = []
while True:
    filename = "notes4/"+str(name)+".txt" #membaca file dari notes4/0.txt, notes4/1.txt
    try:
        ...
        tags = note[2].split(' ') # memisahkan berdasarkan karakter tertentu
        note[2] = tags
