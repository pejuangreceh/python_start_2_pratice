...
questions_list.append(Question('Ibukota negara Jepang?', 'Tokyo', 'Kyoto', 'Osaka', 'Hiroshima'))
questions_list.append(Question('Berapa jumlah planet di Tata Surya?','8', '9', '10', '7'))
questions_list.append(Question('Apa nama senyawa kimia untuk H2O?', 'Air', 'Oksigen', 'Hidrogen', 'Nitrogen'))
questions_list.append(Question('Siapakah penulis novel "Harry Potter"?',  'J.K. Rowling', 'Stephen King', 'George R.R. Martin', 'J.R.R. Tolkien'))
questions_list.append(Question('Apa ibukota negara Italia?', 'Roma', 'Milan', 'Florence', 'Venice'))
questions_list.append(Question('Apa yang menjadi simbol kimia untuk emas?',  'Au', 'Ag', 'Fe', 'Cu'))
questions_list.append(Question('Apa nama samaran untuk Planet Mars?', 'Planet Merah', 'Planet Biru', 'Planet Hijau', 'Planet Kuning'))
questions_list.append(Question('Apa yang merupakan organ terbesar di tubuh manusia?', 'Kulit', 'Jantung', 'Hati', 'Paru-paru'))
questions_list.append(Question('Siapakah presiden pertama Amerika Serikat?',  'George Washington', 'Thomas Jefferson', 'Abraham Lincoln', 'John F. Kennedy'))
questions_list.append(Question('Apa yang menjadi simbol kimia untuk perak?', 'Ag', 'Au', 'Pt', 'Cu'))



# questions_list.append('coba deh')
...
def check_answer():
    ...
    if answers[0].isChecked():
        # a correct answer!
        show_correct('Benar')
        window.skor +=1 #jika jawaban benar, skornya ditambah 1
    else:
...
def next_question():
    ...
    if window.cur_question >= len(questions_list): # jika index pertanyaan sudah mencapai maksimal, maka indexnya diubah paksa ke awal 
        window.cur_question = 0 # if the list of questions has ended, start over 
        print('Skor anda adalah = ',window.skor / len(questions_list) * 100) # ketika pertanyaan habis, tampilkan skornya
        app.closeAllWindows()
    q = questions_list[window.cur_question] # take a question
    print('ini adalah urutan pertanyaan == ',window.cur_question)
    print('Jawaban yang benar = ',window.skor) #tampilkan skornya

    if lb_Result.text() == 'Correct':
        score += 1
    ask(q) # ask it
...
window = QWidget()
window.skor = 0 #bikin variable skor
