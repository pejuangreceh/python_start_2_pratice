...
#App functionality
var1, var2, var3 = 5,"Elloy",True

#Working with note text
def add_note():
    # membuka window baru -> untuk input
...
def show_note():
    ...
    # menampilkan text dan tags yang ada pada notes dengan key yang dipilih
...
def save_note():
    # kondisi jika sedang memilih note
    # 
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        # buat check nama key yang dipilih
...
def del_note():
    # kondisi sama kayak yang save tapi ini buat hapus note
    # variable notes[key] langsung dihapus dan clear semua tampilan 
...
def add_tag():
    # cek kondisi masih sama dengan save_note
    
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        tag = field_tag.text()
        # jika tag tidak ada pada notes, maka tag akan disimpan
        if not tag in notes[key]["tags"]:
            notes[key]["tags"].append(tag)
            list_tags.addItem(tag)
            field_tag.clear()
        # kalo tag sudah ada, maka gaakan disimpan
        else:
            print('tag sudah ada')
        with open("notes3/notes_data.json", "w") as file:
            json.dump(notes, file, sort_keys=True, ensure_ascii=False)
...
def search_tag():
    # check log
    print('awal : ',button_tag_search.text())
    tag = field_tag.text()
    if button_tag_search.text() == "Search notes by tag" and tag:
        ...
        print('setelah search : ',button_tag_search.text())
        ...
        print('setelah reset : ',button_tag_search.text())
    else:
        pass
